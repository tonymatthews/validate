...
# Bring in data, keep only variables that are needed, merge in weights, and create daily rows
  ...
  dat_model <- dat %>%
              ...
              mutate(survtime = ifelse(event_14==0, 14,
                                       date_event-interdat)) %>%
              expandRows("survtime", drop=F) %>%
              mutate(time = sequence(rle(lopnr)$lengths)-1) %>% 
              mutate(event = ifelse(time==survtime-1 & event_14==1, 1, 0)) %>%
  ...
  ipw_exp0 <- data.frame(cbind(seq(0,13),0,(seq(0,13))^2))
  ipw_exp1 <- data.frame(cbind(seq(0,13),1,(seq(0,13))^2))
  ...
  surv0 <- ipw_graph$surv0[14]
  surv1 <- ipw_graph$surv1[14]
  Y_0 <- 1-surv0
  Y_1 <- 1-surv1
  riskdiff <- Y_1 - Y_0
  riskratio <- Y_1/Y_0
  ...
  for (z in 1:numboot) {
    ...
    ipw_exp0_boot <- data.frame(cbind(seq(0,13),0,(seq(0,13))^2))
    ipw_exp1_boot <- data.frame(cbind(seq(0,13),1,(seq(0,13))^2))
  ...
  results_14 <- rbind(results_14, results)
...
results_14 <- results_14 %>%
  ...
  rename("Outcome" = "outcome",
         "Bivalirudin"="Y_1=1", 
         "Heparin"="Y_0=1", 
         "14-day risk difference (95% CI)"="RD", 
         "14-day risk ratio (95% CI)"="RR")
...
write.table(results_14, paste0("logfiles/results_14_ipw_", period), sep="\t", quote=FALSE, row.names=FALSE)
