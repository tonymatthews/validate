...
#Create empty data from for results
...
for(i in outcomes) {
  ...
  dat_model <- dat %>%
    ...
    mutate(survtime = ifelse(event_30==0, 30,
                             date_event-interdat)) %>%
    expandRows("survtime", drop=F) %>%
    mutate(time = sequence(rle(lopnr)$lengths)-1) %>% 
    mutate(event = ifelse(time==survtime-1 & event_30==1, 1, 0)) %>%
    mutate(timesq = time^2)
  
  ###### Model to predict IP weighted risks  ########
  ...
  ipw_exp0 <- data.frame(cbind(seq(0,29),0,(seq(0,29))^2))
  ipw_exp1 <- data.frame(cbind(seq(0,29),1,(seq(0,29))^2))
  ...
  surv0 <- ipw_graph$surv0[30]
  surv1 <- ipw_graph$surv1[30]
  Y_0 <- 1-surv0
  Y_1 <- 1-surv1
  riskdiff <- Y_1 - Y_0
  riskratio <- Y_1/Y_0
  ...
  for (z in 1:numboot) {
    ...
    ipw_exp0_boot <- data.frame(cbind(seq(0,29),0,(seq(0,29))^2))
    ipw_exp1_boot <- data.frame(cbind(seq(0,29),1,(seq(0,29))^2))
  ...
  result2 <- cbind(paste0(outcome1), "Y_0=1", paste0(format(round(Y_0*100,1), nsmall=1) ,
                                              " (" , format(round(lclY_0*100,1), nsmall=1), "," ,
                                                     format(round(uclY_0*100,2), nsmall=1), ")" ))  
  ...
  results_30 <- rbind(results_30, results)
...
write.table(results_30, paste0("logfiles/results_30_", period), sep="\t", quote=FALSE, row.names=FALSE)
