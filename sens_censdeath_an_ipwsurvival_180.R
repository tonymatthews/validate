...
senscensor_results_180 <- data.frame()

# only using MI and bleed as these are the analyses where need to censor at death
outcomes_sens <- c("mi", "bleed")

for(i in outcomes_sens) {
  ...
  #Expand to a row per day until event, and censor at death (if not outcome)
  dat_model <- dat %>%
                merge(death, by="lopnr") %>% 
                merge(ipweights, by="lopnr") %>%
                mutate(endoffup = pmin(date_event, date_death, na.rm=TRUE)) %>% 
                mutate(survtime = endoffup-interdat) %>% 
                mutate(survtime = replace(survtime, survtime>180, 180)) %>% 
                mutate(survtime = replace(survtime, is.na(survtime), 180)) %>%
  ...
  senscensor_results_180 <- rbind(senscensor_results_180, results)
  
} # end for(i in outcomes_sens)

# organise final results
senscensor_results_180 <- senscensor_results_180 %>%
...
rm(boot_dat, boot_dat1, ipw_boot, ipw_exp0, ipw_exp0_boot, ipw_exp1,
   ipw_exp1_boot, ipw_graph, res, result1, result2, result3, result4, results,
   Y_0, Y_1, i, index, ipw_model, ipw_model_boot, lclY_0, 
   lclY_1, lcldiff, outcome, outcome1, dat_model, ipweights, dat,
   res_sd, surv0, surv1, uclY_0, uclY_1, ucldiff, z)

#write the results to a txt file
write.table(senscensor_results_180, paste0("logfiles/sens_censdeath_resuts_180_", period), sep="\t", quote=FALSE, row.names=FALSE)
